...
Type
  ...
  TZAbstractObject = class
    ...
    function equals(const Value: TZAbstractObject): Boolean; reintroduce; virtual;
...
begin
  result := ParseException.create(_message + toString() + ' próximo a : ' + copy(toString(), self.myIndex, 10),
    self.myIndex);
end;
  ...
  begin
    ...
    case (c) of
      ...
      #13: begin
        ...
        if not CharInSet(c, NoConversion) then
        begin
...
var
  ...
  ja: TJSONArray;
begin
  if ((aNames = nil) or (aNames.length() = 0)) then
  begin
    result := nil;
    exit;
  end;
  ja := TJSONArray.create;
  for i := 0 to aNames.length - 1 { ; i < names.length(); i += 1) } do
  begin
...
begin
  ...
  begin
    if (myHashMap.Objects[0] <> CONST_FALSE) and (myHashMap.Objects[0] <> CONST_TRUE) and
